...
| DropKey + streamlined | configs written 2026-09-25 | `local_pack_flex_dropkey` (per-(q,k) hash DropKey in the flex score_mod; free at model level with the 1-round hash). Arms: `..._glob400_dropkey.yaml` (dropout only) and `..._glob400_dropkey_streamlined.yaml` (+ no downward refresh, upward every 4: -18.8% step, -6.3M params). L0 reads L3-L6 directly via the 170-row global block; L1/L2 only via the band; coarse rows never read L0 beyond their band, hence upward kept. |
| One-softmax hierarchy | built + verified 2026-09-25 | Coarse call (`local_pack_level_windows`, `local_pack_down_highway: children|l0`): 2nd flex call over coarse rows in (level, position) order, exact LSE-merged into the union. Restores the lane, which was a SILENT NO-OP on every global-block flexhier arm (prefix mask branch has no lane clause). `scripts/check_coarse_call.py` ALL PASS (disjoint, exact vs dense 2.9e-3, semantics 40/40, causal 0 vs leak 0.48). Arm `..._glob400_dropkey_highway.yaml` (no up/down refresh + lane32 + children): -12.9% step @4k, -6.5% @32k, -12.6M params. Folding the clauses into the single packed union measured +150%/+300% (rejected); a level-grouped main layout measured 2x the kernel (rejected). |
